...
import h5py
import taichi as ti
from experiments.fitting.utils.ground_truth.IterativeEikonal.eikivp.SO3.Riemannian.interpolate import (
    vectorfield_trilinear_interpolate_LI,
    scalar_trilinear_interpolate,
)
from experiments.fitting.utils.ground_truth.IterativeEikonal.eikivp.SO3.utils import (
    get_next_point,
...
from experiments.fitting.utils.ground_truth.IterativeEikonal.eikivp.SO3.costfunction import (
    CostSO3,
)
...
class GeodesicSO3Riemannian:
    ...
    def __init__(self, W: DistanceSO3Riemannian, target_point=None, dt=1.0):
    ...
    def compute_γ_path(
        ...
        self.γ_path = geodesic_back_tracking(
            W.grad_W,
            self.source_point,
            self.target_point,
            C.C,
    ...
    def import_γ_path(self, folder):
        ...
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            assert (
                np.all(self.σ_s_list == geodesic_filename.attrs["σ_s_list"])
                and self.σ_o == geodesic_filename.attrs["σ_o"]
                and self.σ_s_ext == geodesic_filename.attrs["σ_s_ext"]
                and self.σ_o_ext == geodesic_filename.attrs["σ_o_ext"]
                and self.image_name == geodesic_file.attrs["image_name"]
                and self.λ == geodesic_file.attrs["λ"]
                and self.p == geodesic_file.attrs["p"]
                and np.all(self.G == geodesic_file.attrs["G"])
                and np.all(self.source_point == geodesic_file.attrs["source_point"])
                and np.all(self.target_point == geodesic_file.attrs["target_point"])
                and (
                    self.dt == geodesic_file.attrs["dt"]
...
class GeodesicMultiSourceSO3Riemannian:
    ...
    def __init__(self, W: DistanceMultiSourceSO3Riemannian, target_point=None, dt=1.0):
        # Vesselness attributes
        self.σ_s_list = W.σ_s_list
        self.σ_o = W.σ_o
        self.σ_s_ext = W.σ_s_ext
    ...
    def compute_γ_path(
        self,
        W: DistanceMultiSourceSO3Riemannian,
        C: CostSO3,
        α_min,
        β_min,
        φ_min,
        ...
        self.γ_path = geodesic_back_tracking_multi_source(
            W.grad_W,
            self.source_points,
            self.target_point,
            C.C,
    ...
    def import_γ_path(self, folder):
        ...
        with h5py.File(geodesic_filename, "r") as geodesic_file:
            assert (
                ...
                and self.p == geodesic_file.attrs["p"]
                and np.all(self.G == geodesic_file.attrs["G"])
                and np.all(self.source_points == geodesic_file.attrs["source_points"])
                and np.all(self.target_point == geodesic_file.attrs["target_point"])
                and (
                    self.dt == geodesic_file.attrs["dt"]
                    or geodesic_file.attrs["dt"] == "default"
