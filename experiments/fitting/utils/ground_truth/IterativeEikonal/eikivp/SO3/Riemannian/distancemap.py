...
from experiments.fitting.utils.ground_truth.IterativeEikonal.eikivp.SO3.derivatives import (
    upwind_derivatives,
)
...
from experiments.fitting.utils.ground_truth.IterativeEikonal.eikivp.SO3.Riemannian.metric import (
    invert_metric,
)
from experiments.fitting.utils.ground_truth.IterativeEikonal.eikivp.utils import (
    get_initial_W,
...
class DistanceSO3Riemannian:
    ...
    def import_W(self, folder):
        ...
        with h5py.File(distance_filename, "r") as distance_file:
            assert (
                np.all(self.σ_s_list == distance_file.attrs["σ_s_list"])
                and self.σ_o == distance_file.attrs["σ_o"]
                and self.σ_s_ext == distance_file.attrs["σ_s_ext"]
                and self.σ_o_ext == distance_file.attrs["σ_o_ext"]
                and self.image_name == distance_file.attrs["image_name"]
                and self.λ == distance_file.attrs["λ"]
                and self.p == distance_file.attrs["p"]
                and np.all(self.G == distance_file.attrs["G"])
                and np.all(self.source_point == distance_file.attrs["source_point"])
                and (
                    np.all(self.target_point == distance_file.attrs["target_point"])
                    or distance_file.attrs["target_point"] == "default"
                )
    ...
    def compute_W(
        ...
        References:
            [1]: E. J. Bekkers, R. Duits, A. Mashtakov, and G. R. Sanguinetti.
              ...
              DOI:10.1137/15M1018460.
        """
        W, grad_W = eikonal_solver_multi_source(
            C.C,
            self.source_points,
            self.G,
            dα,
    ...
    def import_W(self, folder):
        ...
        with h5py.File(distance_filename, "r") as distance_file:
            assert (
                ...
                and self.p == distance_file.attrs["p"]
                and np.all(self.G == distance_file.attrs["G"])
                and np.all(self.source_points == distance_file.attrs["source_points"])
                and (
                    np.all(self.target_point == distance_file.attrs["target_point"])
                    or distance_file.attrs["target_point"] == "default"
                )
