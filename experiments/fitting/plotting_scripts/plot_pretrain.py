...
df_eikonal = pd.read_csv("./experiments/fitting/results/eiko_pretrain.csv")
df_mse = pd.read_csv(
    "./experiments/fitting/results/mse_pretrain.csv"
)  # Replace with your actual MSE data file

# Create figure with two subplots side by side
fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(10.14 * 1.2, 3.52 * 1.2), dpi=100)
...
def create_eikonal_plot(ax, df):
    y_column1 = "best style a meta - train_eiko_epoch"
    y_column2 = "best style a mix - train_eiko_epoch"
    ...
    (line2,) = ax.plot(
        ...
        label="Styele-A Pretrain",
    )

    y_column1 = "best curve a meta - train_eiko_epoch"
    y_column2 = "best curve a mix - train_eiko_epoch"
    ...
    (line1,) = ax.plot(
        ...
        df[y_column1],
        linewidth=2,
        color="#8B7EC8",  # Purple color
        label="CurveVel-A Vanilla",
    )
    ...
    (line2,) = ax.plot(
        ...
        df[y_column2],
        linewidth=2,
        color="#879A39",  # Green color
        label="CurveVel-A Pretrain",
    )
    ...
    ax.set_ylabel("Eikonal Loss (log scale)")
...
def create_mse_plot(ax, df):
    y_column1 = "best style a meta - train_mse_epoch"
    y_column2 = "best style a mix - train_mse_epoch"
    ...
    (line2,) = ax.plot(
        ...
        linewidth=2,
        color="#4385BE",  # Blue color
        label="Style-A Pretrain",
    )

    y_column1 = "best curve a meta - train_mse_epoch"
    y_column2 = "best curve a mix - train_mse_epoch"
...
plt.savefig("./experiments/fitting/figures/pretrain.pdf", bbox_inches="tight")
